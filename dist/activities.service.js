...
let ActivitiesService = class ActivitiesService {
    ...
    async verifyProfileOwnership(userId, profileId) {
        ...
        if (!profile) {
            throw new common_1.ForbiddenException('Perfil no encontrado o no tienes acceso');
        }
        return profile;
    }
    async getActivitiesByProfile(userId, profileId) {
    ...
    async createActivity(userId, platform, body) {
        const profileId = Number(body.profile_id);
        if (!profileId) {
            throw new common_1.BadRequestException('profile_id es requerido');
        }
        await this.verifyProfileOwnership(userId, profileId);
        ...
        switch (platform.toLowerCase()) {
            ...
            default:
                throw new common_1.BadRequestException('Plataforma no soportada. Usar: youtube, quora, medium, browser, google, gmaps');
        }
    }
    async updateActivity(userId, platform, id, body) {
        ...
        if (plat === 'youtube') {
            ...
            throw new common_1.BadRequestException('Plataforma no válida');
        }
        if (!activity) {
        ...
        const updateData = {};
        if (body.search_query !== undefined)
            updateData.search_query = body.search_query;
        if (body.publish_date !== undefined)
            updateData.publish_date = body.publish_date;
        if (body.publish_time !== undefined)
            updateData.publish_time = body.publish_time;
        ...
        if (plat === 'youtube') {
            if (body.video_id !== undefined)
                updateData.video_id = body.video_id;
            if (body.comment_video_id !== undefined)
                updateData.comment_video_id = body.comment_video_id;
            if (body.likes_video_id !== undefined)
                updateData.likes_video_id = body.likes_video_id;
            return this.prisma.youtubeActivity.update({ where: { id }, data: updateData });
        }
        else if (plat === 'quora') {
            if (body.question_id !== undefined)
                updateData.question_id = body.question_id;
            if (body.answer_id !== undefined)
                updateData.answer_id = body.answer_id;
            if (body.upvote_question_id !== undefined)
                updateData.upvote_question_id = body.upvote_question_id;
            if (body.upvote_answer_id !== undefined)
                updateData.upvote_answer_id = body.upvote_answer_id;
            if (body.comment_question_id !== undefined)
                updateData.comment_question_id = body.comment_question_id;
            if (body.comment_answer_id !== undefined)
                updateData.comment_answer_id = body.comment_answer_id;
            return this.prisma.quoraActivity.update({ where: { id }, data: updateData });
        }
        else if (plat === 'medium') {
            if (body.post_id !== undefined)
                updateData.post_id = body.post_id;
            if (body.claps_post_id !== undefined)
                updateData.claps_post_id = body.claps_post_id;
            if (body.comment_post_id !== undefined)
                updateData.comment_post_id = body.comment_post_id;
            ...
            if (body.place_id !== undefined)
                updateData.place_id = body.place_id;
            if (body.navigate_location !== undefined)
                updateData.navigate_location = body.navigate_location;
    ...
    async updateActivityStatus(userId, platform, id, status) {
        return this.updateActivity(userId, platform, id, { status });
    }
    async finishActivity(userId, platform, id) {
        return this.updateActivity(userId, platform, id, { status: 1, finished_at: new Date() });
    }
    async deleteActivity(userId, platform, id) {
        const plat = platform.toLowerCase();
        let activity = null;
        if (plat === 'youtube') {
            activity = await this.prisma.youtubeActivity.findUnique({ where: { id } });
